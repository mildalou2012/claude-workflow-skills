...
description: >-
  【技能·收尾整理】
  知识与治理收尾：把项目文档、规则文件（CLAUDE.md/AGENTS.md）、授权记忆和工作区残留，与代码和运行时的真实情况对齐，让下一个会话或下一个人从同一个最新答案开始。
  触发词：neat-freak、洁癖、/neat——以及没有点名但明显是收尾意图的场景：开发后同步整理项目文档/规则/记忆（「把文档和记忆整理一下」「收尾时把文档同步掉」「docs 和代码对不上了」）、CLAUDE.md/记忆过期或冲突、向队友或新会话干净交接、审查工作区规则是否真的被执行。
  不触发：纯编码/重构/调试任务、整理数据或文字（JSON、周报、changelog 公告）、或没有项目知识背景的单纯「整理」。
